...
class CentralAgentEnvironment(Env):
    def __init__(self, numByzantine = 1, numHonest = 1, attackType = None):
        class Net(nn.Module):    
        ...
        # print(test.data[0])
        centralServer = Server.CentralServer(test, 128,  Net, val = True)
        self.num_workers = numHonest
        ...
        for i in range(self.num_Byzantine):
            centralServer.addAgent(Agents.Agent(i + self.num_workers, centralServer, True, attackType))
...
#         self.observation_space = spaces.Dict({'gradient' : spaces.Box(low = np.full([arr.shape[0]], -np.inf), high=np.full([arr.shape[0]], np.inf)),
#                                                 'model_loss' : spaces.Box(low=np.array([0]), high=np.array([len(self.centralServer.agentList)])), 
#                                                 'prior' : spaces.Box(low=np.array([0]), high=np.array([1])),
#                                                 'prevRewards' : spaces.Box(low = np.array([-np.inf]), high = np.array([np.inf]))
#                                               })
...
#             print(a.shape, b.shape, c.shape)
    ...
    def reset(self):
            ...
            self.numSteps = int(500 / (1 - ((self.num_Byzantine)/(self.num_Byzantine + self.num_workers))))
#             self.numSteps = 1
            obs, reward, id = self.centralServer.generateObservation()
